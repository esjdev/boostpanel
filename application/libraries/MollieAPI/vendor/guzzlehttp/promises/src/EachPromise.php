 ...
 */
class EachPromise implements \_PhpScoper5e55118e73ab9\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    /** @var \Iterator */
    private $iterable;
    ...
    {
        $this->mutex = \false;
        $this->aggregate = new \_PhpScoper5e55118e73ab9\GuzzleHttp\Promise\Promise(function () {
            \reset($this->pending);
            ...
            while ($promise = \current($this->pending)) {
                \next($this->pending);
                $promise->wait();
                if ($this->aggregate->getState() !== \_PhpScoper5e55118e73ab9\GuzzleHttp\Promise\PromiseInterface::PENDING) {
                    return;
                }
            }
        });
